...
function loadXLSX(){if(window.XLSX)return Promise.resolve(window.XLSX);if(xlsxPromise)return xlsxPromise;xlsxPromise=new Promise((resolve,reject)=>{const script=document.createElement("script");script.src="https://cdn.sheetjs.com/xlsx-0.20.3/package/dist/xlsx.full.min.js";const timeout=setTimeout(()=>reject(Error("The Excel reader could not load. Try again or upload CSV.")),15000);script.onload=()=>{clearTimeout(timeout);window.XLSX?resolve(window.XLSX):reject(Error("Excel reader unavailable. Upload CSV instead."));};script.onerror=()=>{clearTimeout(timeout);reject(Error("The Excel reader could not load. Upload CSV instead."));};document.head.appendChild(script);}).catch(e=>{xlsxPromise=null;throw e;});return xlsxPromise;}
...
async function auctionPage(){
 ...
 const top=C.highest(a),o=C.outcome(a,account?.id,last?.serverNow||Date.now());
 ...
 const win=o==="won";
 return '<span class="signal-light '+(win?"signal-green":"signal-red")+'" role="img" aria-label="'+(win?"Green light: winning bid":"Red light: no winning bid")+'"></span><div><strong>'+(win?"Your bid won":o==="reserve-not-met"?"Reserve not met":"Your bid did not win")+'</strong><p>'+(win?"Winning offer: "+cash(top.amount)+". Seller acceptance and closing are still required.":o==="reserve-not-met"?"No bid qualified at the seller’s minimum.":"Highest offer: "+cash(top?.amount)+".")+"</p></div>";
 }
 async function refresh(){
 ...
 $("bid-form").hidden=!eligible||a.status!=="active";$("bid-register").hidden=!!eligible||a.status!=="active";$("bid-register").href="buyer.html?auction="+encodeURIComponent(a.id)+"&address="+encodeURIComponent(a.title)+"&price="+C.nextBid(a);
 const next=C.nextBid(a);$("bid-amount").min=String(next);if(document.activeElement!==$("bid-amount")&&(!field("bid-amount")||Number(field("bid-amount"))<next))$("bid-amount").value=String(next);$("bid-minimum").textContent="Next bid: "+cash(next)+" or more. Each new bid must exceed the highest offer by at least $100.";
 const outcome=C.outcome(a,account?.id,last?.serverNow||Date.now());
 $("buyer-status").textContent=a.status==="active"?(outcome==="leading"?"You currently lead. The result is determined when time expires.":outcome==="outbid"?"Another buyer has placed a higher bid.":"Your submitted interest is not automatically placed as a bid."):"";
 $("seller-private").hidden=!isSeller;$("seller-access-message").textContent=isSeller?"Your minimum is protected by the required bid. An auction result does not itself charge the success fee.":"Only the listing seller can view all bids and the seller’s proceeds. "+(S.demo?"For a sample listing, choose Test Seller above.":"Use the same browser tab used to submit this listing.");
 if(isSeller){$("seller-net-min").textContent=cash(a.minimum);$("seller-reserve").textContent=cash(a.reserve);$("seller-proceeds").textContent=top?cash(summary.proceeds):"—";$("seller-additional").textContent=cash(summary.additional);$("seller-fee-due").textContent=cash(summary.feeDue);$("complete-sale").hidden=!S.demo||!a.winnerId||a.saleCompleted;}
